...
export class RolesAuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<any> {
        ...
        // Check roles
        if (req.user) {
            const permissions = req.user.permissions || req.user.roles || [];
            return permissions.some(role => requiredPermissions.includes(role));
        }
        ...
        if (isProd) {
            ...
            throw new ForbiddenException({
                message: `Нет прав для выполнения данного действия. (${requiredPermissions.join(', ')})`,
            });
        }
    }
}
